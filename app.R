...
server <- function(input, output, session) {
  ...
  currency_format = eventReactive(input$country,
    ...
    filter(currencies, country==input$country) %>% 
      ...
      select(prefix, suffix, big.mark, decimal.mark) %>%
    as.list())
  
  #observe(print(currency_format()))
  ...
  observe({
    ...
    revenue_p1 = ggplotly(revenue() %>%
      ...
      scale_y_continuous(labels = function(y) do.call(scales::number, c(list(x=round(y,0)), currency_format()[c("big.mark", "decimal.mark")]))) +
      facet_wrap(~facet) +
